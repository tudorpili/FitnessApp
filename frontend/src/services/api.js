...
export const deleteCurrentUserAccount = async (confirmationText) => { const token = localStorage.getItem('authToken'); if (!token) return Promise.reject(new Error("Authentication token not found.")); return apiRequest('/users/me', 'DELETE', { confirmationText: confirmationText }, token); };
