...
import { FiUsers, FiClipboard, FiGrid, FiMenu, FiX, FiLogOut, FiUser } from 'react-icons/fi'; // Added FiUser

function AdminLayout() {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  // --- NEW: State/Ref for profile dropdown ---
  ...
  const navLinkClass = ({ isActive }) => /* ... NavLink styles ... */ `flex items-center px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 group ${ isActive ? 'bg-gradient-to-r from-slate-700 to-slate-900 text-white shadow-inner scale-[1.02]' : 'text-slate-600 hover:bg-slate-200/60 hover:text-slate-900' }`;
  ...
  const sidebarNavigation = (
     <nav className="flex-1 space-y-1.5 px-3">
        ...
        <div className="pt-4 mt-4 border-t border-slate-200">
             ...
             {/* Logout moved to header dropdown */}
             {/* <NavLink to="/login" className={navLinkClass} onClick={() => setIsSidebarOpen(false)}> <FiLogOut className="mr-3 h-5 w-5 flex-shrink-0" /> <span className="truncate">Logout (to Login)</span> </NavLink> */}
        </div>
     </nav>
  );
        ...
        <div className="flex flex-col flex-1 w-full overflow-y-auto">
            ...
            <div className="sticky top-0 z-30 flex-shrink-0 flex h-16 bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-200/80">
                ...
                <div className="flex-1 px-4 flex justify-end items-center">
                    {/* --- NEW: Admin Profile Menu Section --- */}
                    <div className="ml-4 flex items-center md:ml-6 relative" ref={profileMenuRef}>
                        ...
                        <div className={`absolute right-0 mt-2 w-48 origin-top-right rounded-xl bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none transition ease-out duration-100 ${isProfileMenuOpen ? 'transform opacity-100 scale-100 z-40' : 'transform opacity-0 scale-95 pointer-events-none'}`} role="menu" aria-orientation="vertical" tabIndex="-1">
                            ...
                            {/* No Settings link for Admin here maybe? Or link to admin settings? */}
