...
const UnitToggle = ({ selectedUnit, onUnitChange }) => ( /* ... Toggle JSX ... */
...
const convertToCSV = (dataArray, headers) => {
    ...
    // Use provided headers or generate from keys of the first object
    ...
    const rows = dataArray.map(obj => {
        return headerKeys.map(key => {
            ...
            // Handle nested objects/arrays (e.g., sets in workouts) - basic stringify
...
function SettingsPage() {
    ...
    const handleExportWorkouts = () => {
        const headers = ['date', 'name', 'duration_seconds', 'notes', 'exercises_json']; // Define explicit headers
        const csv = convertToCSV(mockWorkoutExportData.map(log => ({ // Map data to match headers
            date: log.date,
            name: log.name,
            duration_seconds: log.duration,
            notes: log.notes,
            exercises_json: log.exercises // Keep exercises as JSON string for simplicity
        })), headers);
        if (csv) {
     ...
     const handleExportWeight = () => {
        const csv = convertToCSV(mockWeightExportData); // Use default headers
        if (csv) {
     ...
     const handleExportMeals = () => {
        const csv = convertToCSV(mockMealExportData); // Use default headers
        if (csv) {
    ...
    return (
        <div className="space-y-8 sm:space-y-10 max-w-4xl mx-auto animate-fadeIn">
            ...
            <InfoCard title="Profile Information" icon={<FiUser />}>
                {/* ... Profile form elements remain the same ... */}
            ...
            <InfoCard title="Preferences" icon={<FiSettings />}>
                 {/* ... Preferences elements remain the same ... */}
            ...
            {/* --- END NEW --- */}
             ...
             <InfoCard title="Account Actions" icon={<FiAlertTriangle />}>
                 {/* ... Account action buttons remain the same ... */}
