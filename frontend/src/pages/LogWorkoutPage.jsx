...
// Removed: import { mockExercises } from '../mockData/exercises'; // Using API now
import { getAllExercises, logWorkout } from '../services/api'; // Import API functions
import {
    ...
    FiMaximize2, FiLoader, FiAlertCircle // Added Loader, AlertCircle
} from 'react-icons/fi';
import { FaDumbbell, FaWeightHanging } from 'react-icons/fa';

// --- Helper Functions & Components (getTodayDate, formatTime, InfoCard, LoggedExerciseEntry) ---
...
function LogWorkoutPage() {
    ...
    // State for displaying finished logs (optional, session only)
    ...
    useEffect(() => {
        const fetchAllExercises = async () => {
            try {
                console.log("Fetching all exercises for search...");
                const data = await getAllExercises(); // Using the API service
                ...
                console.log("All exercises loaded:", parsedData);
            } catch (error) {
    ...
    // --- Timer Logic ---
    useEffect(() => { /* ... (timer logic remains the same) ... */ if (isTimerRunning) { timerIntervalRef.current = setInterval(() => { setElapsedSeconds(prev => prev + 1); }, 1000); } else { clearInterval(timerIntervalRef.current); } return () => clearInterval(timerIntervalRef.current); }, [isTimerRunning]);
    ...
    }, [searchTerm, currentExercises, allExercises]);

    // --- Handlers for Modifying Current Workout ---
    ...
    // --- Handle Finishing and Logging Workout (API Call) ---
    const handleFinishWorkout = async () => { // Make async
        handlePauseTimer(); // Pause timer when attempting to save
        setSaveError(null);
        ...
        const validExercises = currentExercises
            .map(ex => ({
                ...ex,
                // Filter out sets that have no reps OR no weight (adjust if needed)
                sets: ex.sets.filter(set => (set.reps && set.reps > 0) || (set.weight && set.weight > 0))
            }))
            .filter(ex => ex.sets.length > 0); // Only include exercises with valid sets

        if (validExercises.length === 0) {
            setSaveError("Please add at least one exercise with valid set details (reps or weight > 0) before finishing.");
            return;
        }
        ...
        const workoutData = {
            ...
            name: workoutName.trim() || null, // Send null if empty
            notes: workoutNotes.trim() || null, // Send null if empty
            durationSeconds: elapsedSeconds > 0 ? elapsedSeconds : null, // Send null if timer wasn't used
            exercises: validExercises.map(ex => ({
                exerciseId: ex.exerciseId,
                // Map sets to the structure expected by the backend
                sets: ex.sets.map(set => ({
                    reps: set.reps ? parseInt(set.reps, 10) : null, // Ensure integer or null
                    weight: set.weight ? parseFloat(set.weight) : null, // Ensure float or null
                    unit: set.unit || 'kg' // Default unit if needed
                }))
            }))
        };

        console.log("--- Attempting to Log Workout via API ---");
        console.log(JSON.stringify(workoutData, null, 2));

        try {
            // Call the API service function
            const response = await logWorkout(workoutData);
            console.log("Workout logged successfully via API:", response);

            // Add to local display list (optional)
            setFinishedLogs(prevLogs => [{ ...workoutData, logId: response?.session?.id || `local-${Date.now()}` }, ...prevLogs]);
            ...
            console.error("Failed to log workout via API:", error);
            setSaveError(error.message || "An unknown error occurred while saving the workout.");
        } finally {
        ...
        <div className="space-y-10 sm:space-y-12 font-sans max-w-6xl mx-auto animate-fadeIn">
            ...
            <InfoCard title="Workout Timer" icon={<FiClock />}>
                 <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                     <div className={`text-5xl font-mono font-bold text-indigo-700 bg-gradient-to-br from-indigo-50 to-purple-100 px-6 py-3 rounded-2xl shadow-inner ${isTimerRunning ? 'animate-pulse' : ''}`}>{formatTime(elapsedSeconds)}</div>
                     <div className="flex gap-3">
                         {!isTimerRunning ? ( <button onClick={handleStartTimer} disabled={isTimerRunning && elapsedSeconds > 0} className={`flex items-center px-5 py-2.5 rounded-xl text-white font-semibold shadow-md transition duration-200 transform hover:scale-105 ${elapsedSeconds > 0 ? 'bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600' : 'bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600'}`}><FiPlay className="mr-1.5"/> {elapsedSeconds > 0 ? 'Resume' : 'Start'}</button> ) : ( <button onClick={handlePauseTimer} className="flex items-center px-5 py-2.5 rounded-xl bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white font-semibold shadow-md transition duration-200 transform hover:scale-105"><FiPause className="mr-1.5"/> Pause</button> )}
                         <button onClick={handleStopTimer} className="flex items-center px-5 py-2.5 rounded-xl bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white font-semibold shadow-md transition duration-200 transform hover:scale-105"><FiSquare className="mr-1.5"/> Reset</button>
                     </div>
                 </div>
            </InfoCard>
            ...
            {/* Finish Button & Feedback Area */}
            <div className="flex flex-col items-center pt-8 pb-4">
                 {/* Error Message */}
                 {saveError && ( <div className="mb-4 w-full max-w-xl bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative text-sm flex items-center justify-center" role="alert"><FiAlertCircle className="mr-2 h-4 w-4"/> <span className="block sm:inline">{saveError}</span></div> )}
                 {/* Success Message */}
                 {saveSuccess && ( <div className="mb-4 w-full max-w-xl bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg relative text-sm flex items-center justify-center" role="alert"><FiCheckSquare className="mr-2 h-4 w-4"/> <span className="block sm:inline">Workout logged successfully!</span></div> )}
                 {/* Finish Button */}
                 <button onClick={handleFinishWorkout} disabled={currentExercises.length === 0 || isSaving} className="flex items-center justify-center px-10 py-4 border border-transparent text-lg font-semibold rounded-2xl shadow-lg text-white bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition duration-200 ease-in-out transform hover:scale-105 disabled:opacity-60 disabled:cursor-not-allowed">
                    {isSaving ? ( <FiLoader className="animate-spin mr-2.5 h-6 w-6"/> ) : ( <FiCheckSquare className="mr-2.5 h-6 w-6"/> )}
                    {isSaving ? 'Saving Workout...' : 'Finish & Log Workout'}
                </button>
            </div>

            {/* Optional: Display Finished Logs (Session Only) */}
            {finishedLogs.length > 0 && ( <InfoCard title="Saved Workouts (This Session)" icon={<FiList />}> <div className="flex space-x-6 overflow-x-auto pb-4 -mb-4 scrollbar-thin scrollbar-thumb-indigo-200 scrollbar-track-transparent"> {finishedLogs.map(log => ( <div key={log.logId} className="flex-shrink-0 w-72 bg-gradient-to-br from-gray-50 to-white p-5 rounded-2xl border border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-200"> <div className="flex justify-between items-center mb-3"> <p className="font-semibold text-md text-indigo-800 truncate" title={log.name}>{log.name}</p> <span className="text-xs font-medium text-gray-500 bg-gray-200 px-2 py-0.5 rounded-full flex-shrink-0">{log.sessionDate}</span> </div> <p className="text-sm text-gray-600 mb-3">Duration: <span className="font-medium">{formatTime(log.durationSeconds)}</span></p> <details className="text-sm"> <summary className="cursor-pointer text-indigo-600 hover:text-indigo-800 font-medium">View Exercises ({log.exercises.length})</summary> <ul className="mt-2 pl-1 space-y-2 text-gray-700 max-h-40 overflow-y-auto pr-1 scrollbar-thin"> {log.exercises.map(ex => ( <li key={ex.exerciseId} className="border-b border-gray-100 pb-1 last:border-0"> <span className="font-medium text-xs block">{ex.name}</span> <ul className="pl-3 mt-0.5 space-y-0.5"> {ex.sets.map((set, setIndex) => ( <li key={setIndex} className="text-xs text-gray-500 list-disc list-inside"> {set.reps} reps @ {set.weight} {set.unit} </li> ))} </ul> </li> ))} </ul> </details> </div> ))} <div className="flex-shrink-0 w-1"></div> </div> </InfoCard> )}
        </div>
    );
}
