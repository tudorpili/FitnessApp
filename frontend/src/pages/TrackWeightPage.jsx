...
function TrackWeightPage() {
    ...
    // --- NEW: State for chart time range filter ---
    const [chartRange, setChartRange] = useState('all'); // '7d', '30d', '1y', 'all'
    ...
    const summaryStats = useMemo(() => {
        ...
        if (sorted.length >= 2 && !isNaN(lastWeightKgNum)) {
            ...
            if (!isNaN(prevWeightKgNum)) {
                changeKg = (lastWeightKgNum - prevWeightKgNum);
                if (changeKg > 0.05) trend = 'up';
                else if (changeKg < -0.05) trend = 'down';
                else trend = 'stable';
            }
        }
        const displayLastWeight = !isNaN(lastWeightKgNum) ? (displayUnit === 'lbs' ? (lastWeightKgNum * KG_TO_LBS).toFixed(1) : lastWeightKgNum.toFixed(1)) : 'N/A';
        const displayChange = changeKg !== null ? (displayUnit === 'lbs' ? (changeKg * KG_TO_LBS).toFixed(1) : changeKg.toFixed(1)) : null;
        return { lastEntryDate: lastEntry.log_date, lastEntryDisplayWeight: displayLastWeight, displayUnit: displayUnit, displayChange: displayChange, trend: trend };
    }, [weightLog, displayUnit]);

    // --- UPDATED: chartData calculation with filtering ---
    const chartData = useMemo(() => {
        ...
        // Filter the log based on the start date
        const filteredLog = weightLog.filter(entry => {
            if (!startDate) return true; // Include all if range is 'all'
            // Ensure comparison is done correctly (Date objects or reliable string format)
            return new Date(entry.log_date) >= startDate;
        });

        // Sort the filtered data for the chart
        const sortedLogForChart = [...filteredLog].sort((a, b) => new Date(a.log_date) - new Date(b.log_date));

        return {
            labels: sortedLogForChart.map(entry => entry.log_date),
            datasets: [{
                label: 'Weight (kg)',
                data: sortedLogForChart.map(entry => parseFloat(entry.weight_kg) || 0), // Use filtered data
    ...
    }, [weightLog, chartRange]); // Add chartRange as a dependency
    ...
    // --- Event Handlers ---
    const handleAddWeight = async (e) => { /* ... (remains the same) ... */ e.preventDefault(); setFormError(''); setSaveSuccess(false); const weightValue = parseFloat(weightInput); if (isNaN(weightValue) || weightValue <= 0) { setFormError('Please enter a valid weight > 0.'); return; } if (!dateInput) { setFormError('Please select a date.'); return; } setIsSaving(true); try { const response = await addOrUpdateWeight(dateInput, weightValue, unitInput); console.log("Weight log saved/updated:", response); fetchHistory(); setWeightInput(''); setFormError(''); setSaveSuccess(true); setTimeout(() => setSaveSuccess(false), 3000); } catch (error) { console.error("Failed to save weight log:", error); setFormError(error.message || "An error occurred while saving."); } finally { setIsSaving(false); } };
    const handleDeleteWeight = async (idToDelete) => { /* ... (remains the same) ... */ console.log(`Attempting to delete weight entry with id: ${idToDelete}`); try { await deleteWeightLog(idToDelete); console.log(`Weight log ${idToDelete} deleted successfully.`); fetchHistory(); } catch (error) { console.error(`Failed to delete weight log ${idToDelete}:`, error); alert(`Error deleting entry: ${error.message}`); } };
    ...
    return (
        <div className="space-y-8 sm:space-y-10 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-1 rounded-lg">
            ...
            {isLoading && ( <div className="flex justify-center items-center py-10"> <FiLoader className="animate-spin h-8 w-8 text-indigo-600" /> <span className="ml-3 text-gray-600">Loading History...</span> </div> )}

            {/* Fetch Error State */}
            {fetchError && !isLoading && ( <div className="text-center py-10 px-6 bg-red-50 rounded-2xl shadow border border-red-200/50"> <FiAlertCircle className="mx-auto h-8 w-8 text-red-500 mb-3"/> <h3 className="text-lg font-semibold text-red-700 mb-2">Failed to Load History</h3> <p className="text-red-600 text-sm mb-4">{fetchError}</p> <button onClick={fetchHistory} className="px-4 py-1.5 rounded-md text-sm font-medium bg-red-100 text-red-700 border border-red-200 hover:bg-red-200 transition">Try Again</button> </div> )}
            ...
            {!isLoading && !fetchError && (
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="md:col-span-2"> <InfoCard title="Summary" icon={<FaWeight />} className="h-full"> {weightLog.length > 0 ? ( <div className="space-y-2"> <p className="text-sm text-gray-600"> Last entry: <span className="font-bold text-indigo-700">{summaryStats.lastEntryDisplayWeight} {summaryStats.displayUnit}</span> on {summaryStats.lastEntryDate} </p> {summaryStats.displayChange !== null && ( <p className="text-sm text-gray-600 flex items-center gap-1"> Trend: <TrendIcon trend={summaryStats.trend} /> <span className={`font-semibold ${ summaryStats.trend === 'down' ? 'text-green-600' : summaryStats.trend === 'up' ? 'text-red-600' : 'text-gray-600' }`}> {summaryStats.displayChange > 0 ? `+${summaryStats.displayChange}` : summaryStats.displayChange} {summaryStats.displayUnit} </span> since previous entry. </p> )} {summaryStats.displayChange === null && weightLog.length === 1 && <p className="text-sm text-gray-500 italic">Log another entry to see trend.</p>} </div> ) : ( <p className="text-sm text-gray-500 italic">Log your weight to see a summary.</p> )} </InfoCard> </div>
                        <div className="md:col-span-1"> <InfoCard title="Motivation" icon={<FaQuoteLeft />} className="h-full"> <p className="text-md italic text-gray-700">"{quotes[quoteIndex]}"</p> </InfoCard> </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10 items-start">
                        ...
                        <div className="lg:col-span-1 space-y-8">
                            <InfoCard title="Log Weight" icon={<FiSave />}> <form onSubmit={handleAddWeight} className="space-y-5"> <div> <label htmlFor="weight-date" className="block text-sm font-medium text-gray-700 mb-1">Date</label> <input type="date" id="weight-date" value={dateInput} onChange={(e) => setDateInput(e.target.value)} required className="w-full px-3 py-2.5 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" /> </div> <div> <label htmlFor="weight-value" className="block text-sm font-medium text-gray-700 mb-1">Weight</label> <div className="flex items-center gap-3"> <input type="number" id="weight-value" value={weightInput} onChange={(e) => setWeightInput(e.target.value)} required step="0.1" min="0" placeholder="e.g., 75.5" className={`flex-grow px-3 py-2.5 border rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:border-indigo-500 ${formError && (isNaN(parseFloat(weightInput)) || parseFloat(weightInput) <= 0) ? 'border-red-500 ring-red-500' : 'border-gray-300 focus:ring-indigo-500'}`} /> <UnitToggle selectedUnit={unitInput} onUnitChange={setUnitInput} /> </div> </div> {formError && ( <p className="text-sm text-red-600 flex items-center"><FiAlertCircle className="mr-1"/> {formError}</p> )} {saveSuccess && ( <p className="text-sm text-green-600 flex items-center"><FiCheck className="mr-1"/> Weight logged successfully!</p> )} <button type="submit" disabled={isSaving} className="w-full flex justify-center items-center px-4 py-3 border border-transparent text-sm font-semibold rounded-xl shadow-md text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-200 ease-in-out transform hover:scale-[1.02] disabled:opacity-60"> {isSaving ? <FiLoader className="animate-spin mr-2"/> : <FiSave className="mr-2" />} {isSaving ? 'Saving...' : 'Log Weight'} </button> </form> </InfoCard>
                            <InfoCard title="Weight History" icon={<FiCalendar />}> <div className="flex justify-end mb-3 -mt-2"> <label className="text-xs font-medium text-gray-500 mr-2 self-center">Display unit:</label> <UnitToggle selectedUnit={displayUnit} onUnitChange={setDisplayUnit} /> </div> <div className="max-h-[50vh] overflow-y-auto pr-2 space-y-3"> {sortedLogForList.length > 0 ? ( <ul className="space-y-3"> {sortedLogForList.map(entry => ( <li key={entry.id} className="flex justify-between items-center p-4 bg-white rounded-xl border border-gray-200 shadow-sm animate-fadeIn"> <div> <p className="text-sm font-medium text-gray-500">{entry.log_date}</p> <p className="text-lg font-bold text-indigo-700">{entry.displayWeight} <span className="text-xs font-medium text-gray-500">{entry.displayUnit}</span></p> </div> <button onClick={() => handleDeleteWeight(entry.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-100 rounded-full transition-colors duration-150" title="Delete entry" aria-label="Delete entry"> <FiTrash2 className="h-4 w-4" /> </button> </li> ))} </ul> ) : ( <p className="text-sm text-gray-500 italic text-center py-4">No weight logged yet.</p> )} </div> </InfoCard>
                        </div>

                        {/* Chart Column */}
                        <div className="lg:col-span-2">
                            <InfoCard title="Weight Chart" icon={<FiTrendingUp />}>
                                {/* --- NEW: Chart Range Filter Buttons --- */}
                                <div className="flex justify-center gap-2 mb-4 -mt-2">
                                    {['7d', '30d', '1y', 'all'].map(range => (
                                        ...
                                        >
                                            {range === '7d' ? '7 Days' : range === '30d' ? '30 Days' : range === '1y' ? '1 Year' : 'All Time'}
                                        </button>
                                    ))}
                                </div>
                                {/* --- End Filter Buttons --- */}
                                <div className="h-[65vh] min-h-[300px]">
                                    {/* Show chart only if there's data *after filtering* */}
                                    {chartData.labels && chartData.labels.length >= 2 ? (
                                        <Line options={chartOptions} data={chartData} />
                                    ) : (
                                        <div className="flex items-center justify-center h-full text-gray-500 italic">
                                            {weightLog.length < 2 ? 'Log at least two entries to see the chart.' : 'Not enough data for the selected time range.'}
                                        </div>
                                    )}
                                </div>
                            </InfoCard>
